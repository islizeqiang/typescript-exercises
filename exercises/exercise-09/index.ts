...
type PromisifyOldFunctionDefinition<T> = (
...
function promisify<T>(
  oldFunction: PromisifyOldFunctionDefinition<T>
): PromisifyNewFunctionDefinition<T> {
  return () =>
...
const api = {
  ...
  requestCurrentServerTime: promisify(
...
async function startTheApp() {
  ...
  console.log(
    `   ${new Date(
      await api.requestCurrentServerTime()
    ).toLocaleString()}`
  );
  console.log();
